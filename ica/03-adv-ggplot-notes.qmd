...
The main claim being demonstrated is that across a variety of geographic locations, the Earth is getting hotter. 
  - How did this goal inform what information is displayed in the visualization?
The visualization shows temperature highs and lows for every day in 2015 and compares them to temperature ranges that have been normalized from temperature data between 1981 and 2010. The vizualization also shows monthly precipation in 2015 compared with normal precipitation levels.
...
* months and days of 2015 along the x-axis
* low/high temperature are the boundaries of the red bars
* record high/low and normal range boundaries of grey and brown bars
* logical variable of whether the temperature is record high or not
...
* geom_linerange for high/low temperature, high/low normal temperature, and record high/low for each day, faceted by fill color
  * entire viz also faceted out by different cities selected at the top
* geom_line for precipitation visualization
  * geom_abline for normal precipitation reference line
...
ggplot(weather, aes(x = dateInYear)) +
  geom_linerange(ymin = RecordLow, ymax = RecordHigh, color = '#ECEBE3') +
  geom_linerange(ymin = NormalLow, ymax = NormalHigh, color = '#C8B8BA') +
  theme_classic()
